...
const UNI_REGISTRY: Record<string, { name: string; calendarUrl: string; portalUrl: string; steps: string[]; lms: string }> = {
  ...
  'manchester.ac.uk': {
    ...
    steps: [
      'Click the button below to open your Manchester Blackboard',
  ...
  'ucl.ac.uk': {
    ...
    steps: [
      'Click the button below to open UCL Moodle calendar export',
      'Select "All events" and click "Get calendar URL"',
  ...
  'kcl.ac.uk': {
    ...
    steps: [
      'Click the button below to open your KCL Canvas settings',
      'Scroll down to "Other Feeds" and click "Calendar Feed"',
  ...
  'imperial.ac.uk': {
    ...
    steps: [
      'Click the button below to open Imperial Blackboard',
  ...
  'ox.ac.uk': {
    ...
    steps: [
      'Click the button below to open Oxford Canvas settings',
      'Scroll to "Other Feeds" and click "Calendar Feed"',
      'Copy the URL',
      'Paste it back here',
    ],
  },
  'cam.ac.uk': {
    ...
    steps: [
      'Click the button below to open Cambridge Moodle calendar export',
      'Select "All events" and click "Get calendar URL"',
      'Copy the URL',
      'Paste it back here',
    ],
  ...
  'birmingham.ac.uk': {
    ...
    steps: [
      'Click the button below to open Birmingham Canvas settings',
      'Scroll to "Calendar Feed" under Other Feeds',
      'Copy the URL',
      'Paste it back here',
    ],
  },
...
const GENERIC_OUTLOOK = {
  ...
  steps: [
    'Click the button below to open Outlook Web',
...
function detectUniversity(email: string) {
  ...
  if (domain.endsWith('.ac.uk') || domain.endsWith('.edu')) {
    return { ...GENERIC_OUTLOOK, name: domain, domain }
  }
  return null
}
...
export default function LmsSettings() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('profiles').select('email_domain, university').eq('id', user.id).single()
      .then(({ data }) => {
        if (data?.email_domain) {
          const uni = detectUniversity('x@' + data.email_domain)
  ...
  const handleResync = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('fetch-ics', {
        body: { userId: user?.id, resync: true }
      });
      if (error) throw new Error(error.message);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-sm">{connection.lms_name || connection.university_name}</p>
            ...
            <Card className="glass-card rounded-2xl border-primary/20">
              <CardContent className="p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setDetected(null)} className="text-xs text-muted-foreground underline">
                    Change
                  </button>
                </div>
                ...
                <button
                  onClick={() => window.open(detected.portalUrl, '_blank')}
