      ...
      <div className="px-4 space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <Link to="/plan" className="text-[10px] text-white/30 hover:text-primary transition-colors">Full schedule →</Link>
          </div>
          {loading ? (
            ...
            <div className="space-y-1.5">
              {events.map((event) => {
                ...
                return (
                  <div key={event.id} className="flex items-stretch gap-3 glass-card rounded-xl px-3 py-3">
                    <div className="flex flex-col justify-center min-w-[40px]">
                      <p className="text-xs font-bold text-white/60 leading-none">
                        {format(new Date(event.start_time), 'HH:mm')}
                      </p>
                    </div>
                    <div className="w-0.5 rounded-full shrink-0 self-stretch" style={{ backgroundColor: colour }} />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm text-white leading-tight truncate">{event.title}</p>
        ...
        {teams.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] text-white/30 uppercase tracking-widest font-bold">Your Teams</p>
              <Link to="/social" className="text-[10px] text-white/30 hover:text-primary transition-colors">See all →</Link>
            </div>
